        ...
        , renderPage = function () {
            ...
            */
            ...
            if (setter.pageTabs && !setter.refreshTabls && tabsPage.type === 'nav') {
           	...
           	if (pathURL && pathURL!="/" && $('#LAY_app_tabsheader').find("li[lay-id='"+pathURL+"']").length > 0 ) {
           		var index = $('#LAY_app_tabsheader').find("li[lay-id='"+pathURL+"']").index();
            ...
            if (isIndPage || pathURL === 'login') { //此处单独判断登入页，是为了兼容旧版（即未在 config.js 配置 indPage 的情况）
                ...
                if (setter.interceptor) {
                    ...
                    admin.req({
	                    /*url: '/Account/IsLogin'
						,type:'post'*/
						 url: admin.basePath + '/IsLogin.json'
                    })
                ...
                if (admin.pageType === 'console') { //后台主体页
                	...
                	var url = 'admin/layout' + layui.admin.skin(); 
                	
                    container.render(url).done(function () {
                        renderPage();
