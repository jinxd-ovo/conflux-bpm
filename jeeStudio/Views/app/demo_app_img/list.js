		...
		methods:{
			add:function(){
				...
				$.openWindow({
					...
					url: app.staticPath + 'demo_app_img/content.html' + urlParams,
					show: {
						aniShow: 'pop-in'
					},
					extras: {
	...
	var vm = new Vue({
		...
		methods: {
			reload:function(){
				var _this = this;
			  	app.ajax({
			  		url: app.path + '/app/zform/data/demo_app_img?path=path&length=0', 
			  		type: 'post', 
			  		data: JSON.stringify({
			...
			getList: function(current) {
				...
				app.ajax({
					url: app.path + '/app/zform/data/demo_app_img?path=path&length=' + arrWithOutUnsent.length, 
					type: 'post',
					data: JSON.stringify({
						s01: _this.searchName
					}),
			...
			search: function(){
				var _this = this;
				app.ajax({
					url: app.path + '/app/zform/data/demo_app_img?path=path&length=' + _this.rows.length, 
					type: 'post', 
					data: JSON.stringify({
						s01: _this.searchName
					}),
