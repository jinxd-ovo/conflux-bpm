			...
			getTreeData: function() {
				...
				/*app.ajax({
					...
					success: function(result) {
						setTimeout(function() {
							...
							_this.setPinyin(result.data.userList);
						}, 0);
					}
				});
				...
				app.ajax({
					...
					success: function(result) {
						setTimeout(function() {
							var data = result.data.data;
							_this.setPinyin(result.data.data);
						}, 0);
					}
				});*/
				/*
				app.ajax({
					url: app.path + '/app/area/areaTreeAsync?id=0',
					type: 'post',
					data: '',
					success: function(result) {
						setTimeout(function() {
	...
	var vm = new Vue({
		...
		mounted: function() {
			...
			this.$nextTick(function(){
				this.initWebUploader(this.$refs.uploadfiles01Ref,"s01","uploadfiles01");
			})
		},
		updated: function() {},
		methods: {
			  	...
			  	app.ajax({
			  		...
			  		success: function(result) {
						setTimeout(function() {
							...
							app.ajax({
								url:app.path + '/system/sysFile/getFileList?groupId='+_this.data.s01,
								type:'post',
								success:function(result){
									_this.data.uploadfiles01.remainNum = _this.data.uploadfiles01.remainNum - result.data.fileListMap.files.length;
									$.each(result.data.fileListMap.files, function(i,item) {
										var obj = {
											status: "true",
											src: app.path + "/system/sysFile/fileDownload?fileId=" + item.id,
											image:item.name,
											id: item.id
										}
										_this.data.uploadfiles01.imgList.push(obj);
									});
									mui.previewImage();
								}
							});
				...
				if(needDo=="needDo" || needDo=='sent' || needDo=='done'){
					...
					app.ajax({
						...
						success: function(result) {
							...
							$.openWindow({
								id: 'list' + new Date().getTime(),
								url: app.staticPath + 'demo_app_img/list.html',
								show: {
									aniShow: 'pop-in'
								},
								styles: {
